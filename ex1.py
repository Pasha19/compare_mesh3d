...
def run(
    ...
    vedo.show(transformed_plane.c("green"), restored_plane.c("red")).close()
    transformed_plane.write("M_D.stl")
    restored_plane.write("M_I.stl")
...
def main() -> None:
    ...
    with open(out_tsv_path, "w", newline="") as f:
        ...
        for row in in_tsv.itertuples(index=True):
            ...
            writer.writerow(params + [round(d / (std_vox_size * row.voxel_size), 3) for d in dist])
            n += 1
